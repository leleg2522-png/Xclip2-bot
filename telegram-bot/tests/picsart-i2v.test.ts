...
assert.equal(KLING_MOTION_CONTROL_POOL, null);
...
assert.equal(shouldExportPicsartI2v('grok_imagine'), false);
...
assert.equal(JSON.parse(klingOmniLandscape.options.drive.attributes.aiSDKPayload).aspectRatio, '16:9');
assert.equal(
  extractPicsartVideoUrl({
    status: 'COMPLETED',
...
assert.equal(shouldExportPicsartI2v('pixverse_v6'), false);
...
assert.deepEqual(getPicsartExportSize('9:16', '1080p'), { width: 1080, height: 1920 });
assert.deepEqual(getPicsartExportSize('16:9', '1080p'), { width: 1920, height: 1080 });
...
assert.match(botSource, /picsart_kling_omni: 3500/);
...
assert.match(botSource, /mh3_res_1k/);
assert.doesNotMatch(botSource, /mh3_res_4k/);
assert.match(botSource, /Output: \*768p native\*/);
assert.doesNotMatch(botSource, /hampir selesai menyiapkan video/);
...
assert.match(botSource, /Veo 3\.1 Lite 720p/);
...
assert.doesNotMatch(botSource, /menyiapkan video akhir \$\{opts\.exportResolution\}/);
assert.match(botSource, /Wan 3\.0 480p/);
assert.match(botSource, /Seedance 2\.0 Mini 480p/);
assert.match(botSource, /Seedance 2 Mini Video Edit 480p/);
...
assert.match(botSource, /Seedance 2 Fast Video Edit 480p/);
...
assert.match(botSource, /Seedance 2 Video Edit 480p/);
...
assert.match(botSource, /Seedance 2\.0 Fast 480p/);
assert.match(botSource, /Seedance 2\.0 480p/);
...
assert.match(picsartSource, /SEEDANCE_MAX_REF_IMAGES = 10/);
assert.match(picsartSource, /type: 'video_url'[\s\S]*role: 'reference_video'/);
assert.match(picsartSource, /video\?: \{ buffer: Buffer/);
assert.doesNotMatch(picsartSource, /submitPicsartI2vExport/);
assert.doesNotMatch(picsartSource, /pollPicsartI2vExportResult/);
assert.doesNotMatch(picsartSource, /onStatus\?\.\('export'\)/);
