...
const picsart = readFileSync(new URL('../src/picsart.ts', import.meta.url), 'utf8');
...
assert.match(publicSeedance25Block, /seedance25RatioKeyboard/);
assert.match(publicSeedance25Block, /seedance25_ratio_916/);
assert.match(publicSeedance25Block, /seedance25_ratio_169/);
assert.match(publicSeedance25Block, /'9:16'/);
assert.match(publicSeedance25Block, /'16:9'/);
...
assert.match(picsart, /gw-v2\/workflows\/seedance\/submit/);
assert.match(bot, /resolution: '480p'/);
assert.match(bot, /generateAudio: true/);
assert.match(bot, /const settingsLabel = `\$\{ratio\} · 30 detik · 480p · audio`/);
assert.match(bot, /runPicsartSeedance25\([\s\S]*imageUrls, ratio, videoUrl, videoMime\)/);
