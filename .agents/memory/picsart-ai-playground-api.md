...
- Bot also offers user-selected 16:9, mirrored in both submit `aspect_ratio` and Drive `aiSDKPayload.aspectRatio`. Only 9:16 image-to-video was captured; 16:9 was captured on the text-to-video options endpoint, not a completed image-to-video job. **Why:** The user explicitly requested both ratios. **How to apply:** Keep both fields in sync and do not describe 16:9 image-to-video as live-verified.
...
- GPT Image 2.5 Sunburst/Flare use **different** model strings (`gpt-image-2.5-sunburst`, `gpt-image-2.5-flare`) and `/gw-v2` workflow paths. Captured successful image-to-image jobs submitted to `/gw-v2/workflows/openai-image-editing/submit` but polled through `/gw-v2/workflows/openai-images-generate/{id}/result`, yielding `response.result.urls[0]` and 2 provider credits for high quality. The successful 9:16 payload used `size:"1024x1824"`, `quality:"high"`, `background:"opaque"`, `output_format:"png"`, and Drive `aiSDKPayload` with `imageUrls`, `aspectRatio`, `quality`, `background`, `outputFormat`, `count`; this is NOT the older GPT Image 2 sizing contract. `/gw-v2/workflows/openai-images-generate/options` confirmed both models' text-to-image pricing but no text-only submit was captured. **Why:** submit/poll mismatch and model-specific sizes make blind reuse of the older GPT Image 2 helper fail. **How to apply:** Keep this gateway model's submit and poll paths distinct; treat text-only and >1 reference as not yet live-verified, and do not mistake 2 provider credits for Rp600 bot price.
